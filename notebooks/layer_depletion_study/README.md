# ONDOUSDC Price-Level Survival Study

This folder is for the ONDOUSDC price-level survival study on Binance.
...
- `01_layer_depletion.ipynb`
  ...
  - load ONDOUSDC train days `20260226`, `20260227` and validation day `20260228`
...
- `02_implied_cancellation_proxy.ipynb`
  ...
  - compare visible depth reduction with same-price aggressive trade flow on the validation day
...
- `03_depletion_bias_prediction.ipynb`
  - turn the depletion proxy into a past-only price feature
  - test whether depletion bias predicts `1s`, `2s`, and `3s` forward returns
  - compare depletion bias against top-of-book imbalance on the shared train / validation split
